...
void addHozLineToThermalMap(boost::numeric::ublas::matrix<int> *thermalMap, coord *coord1, coord *coord2);
void addVerLineToThermalMap(boost::numeric::ublas::matrix<int> *thermalMap, coord *coord1, coord *coord2);
...
void updateThermalMap(boost::numeric::ublas::matrix<int> *thermalMap, coord *coord1, coord *coord2)
{
  ...
  {
    addHozLineToThermalMap(thermalMap, coord1, coord2);
  }
  else
  {
    addVerLineToThermalMap(thermalMap, coord1, coord2);
  }
}
...
void addVerLineToThermalMap(boost::numeric::ublas::matrix<int> *thermalMap, coord *coord1, coord *coord2)
{
  if ((*coord1).x < (*coord2).x && (*coord1).y < (*coord2).y)
  {
    for (coord i = {x : (*coord1).x, y : (*coord1).y}; i.x <= (*coord2).x && i.y <= (*coord2).y; i = {x : i.x + 1, y : i.y + 1})
    {
      (*thermalMap)(i.x, i.y) += 1;
    }
  }
