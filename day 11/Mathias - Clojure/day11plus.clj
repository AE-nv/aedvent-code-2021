...
(defn all-flashing [grid]
  (every? (fn [energy-level] (= energy-level 0)) (vals grid)))

(defn count-steps-until
  ([pred world]
   (count-steps-until pred world 0))
  ([pred world step-count]
   (if (pred world)
     step-count
     (recur pred (step world) (inc step-count)))))
...
(comment "repls tests"
  ...
  (def example-grid (parse example-input))
  (sort (first (step-n-times example-grid 195)))
  (step-n-times example-grid 194)

  (count-steps-until (fn [[grid _]] (all-flashing grid)) [{[0 0] 9 [0 1] 8} 0])
  (= 195 (count-steps-until (fn [[grid _]] (all-flashing grid)) [example-grid 0]))
  ,)
...
(defn -main
  "Main function"
  []
  (println (let [initial-world [(parse (util/file->seq "2021/d11.txt")) 0]]
             (count-steps-until (fn [[grid _]] (all-flashing grid)) initial-world))))