...
(defn generate-paths-from [connection-map path-so-far]
  (let [last (peek path-so-far)
        options (->> (connection-map last)
                     (filter (fn [x] (or (util/upper-case? x) (not (already-visited? path-so-far x))))))
...
(defn generate-all-paths
  ([connection-map start end]
   (generate-all-paths connection-map #{} #{[start]} end))
  ([connection-map done todo end]
   (let [[finished unfinished]
         (reduce (fn [[finished unfinished] path]
                   (let [[reached not-reached :as spawned-paths] (->> (generate-paths-from connection-map path)
     ...
     (if (seq unfinished)
       (recur connection-map new-done unfinished end)
       new-done))))

(comment "repls tests"
  "in each point check all options and spawn a path for it
  re-add it to the to-be-processed list

  when a path reached end or dead ends stop and add it to a set of complete paths"
  ((parse example-input) [9 9])

  (def small-connection-map (parse-path small-example))

  (generate-paths-from small-connection-map ["start" "b"])

  (let [[a b] '()]
    b)

  (already-visited? ["a" "b"] "c")

  (generate-all-paths small-connection-map #{} #{[start]} end)
  (generate-all-paths small-connection-map start end)
  ,)
