...
record Grid(ImmutableDictionary<Coordinate, char> Pixels)
{
    ...
    (Coordinate min, Coordinate max) MinMax => _minmax ?? (_minmax = Pixels.Keys.Aggregate((min: new Coordinate(int.MaxValue, int.MaxValue), max: new Coordinate(int.MinValue, int.MinValue)), (acc, v) => (new(Min(acc.min.x, v.x), Min(acc.min.y, v.y)), new(Max(acc.max.x, v.x), Max(acc.max.y, v.y))))).Value;
    IEnumerable<Coordinate> FullRange() =>
        from y in Range(MinMax.min.y - 1, MinMax.max.y - MinMax.min.y + 3) from x in Range(MinMax.min.x - 1, MinMax.max.x - MinMax.min.x + 3) select new Coordinate(x, y);
    private Grid Enhance(string algorithm, bool odd) => new Grid((
        from c in FullRange()
        let index = (
from j in Range(-1, 3) from i in Range(-1, 3) let n = new Coordinate(c.x + i, c.y + j) select Pixels.GetValueOrDefault(n, odd ? '.' : '#')).Aggregate(0, (index, c) => (index << 1) | (c == '#' ? 1 : 0))
